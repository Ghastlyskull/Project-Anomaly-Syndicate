...
namespace ProjectAnomalySyndicate.Quests
{
    ...
    public class QuestNode_Root_PromotionCeremony : QuestNode
    {
        ...
        protected override void RunInt()
        {
            ...
            Thing thing = QuestGen_Shuttle.GenerateShuttle(bestowingFaction); //(bestowingFaction, null, null, acceptColonists: false, onlyAcceptColonists: false, onlyAcceptHealthy: false, -1, dropEverythingIfUnsatisfied: false, leaveImmediatelyWhenSatisfied: false, dropEverythingOnArrival: false, stayAfterDroppedEverythingOnArrival: false, null, null, -1, TransportShipDefOf.Ship_Shuttle);

            Pawn pawn2 = quest.GetPawn(new QuestGen_Pawns.GetPawnParms
            {
                mustBeOfKind = DefOfs.Gha_Syndicate_Royal_Bestower,
            ...
            /*foreach(ThingDef item in addItems)
            {
                innerContainer.TryAdd(ThingMaker.MakeThing(item), 1);
            }*/
            ...
            for (int j = 0; j < 6; j++)
            {
                Pawn item = quest.GeneratePawn(DefOfs.Gha_Syndicate_Fighter_Janissary, bestowingFaction);
                list.Add(item);
                list2.Add(item);
            }
            ...
            List<Thing> itemsReward = new List<Thing>();
            foreach(ThingDef def in addItems)
            {
                itemsReward.Add(ThingMaker.MakeThing(def));
            }
            QuestPart_Choice.Choice item2 = new QuestPart_Choice.Choice
            {
                rewards = { (Reward)new Reward_BestowingCeremonySyndicate
            {
        ...
        protected override bool TestRunInt(Slate slate)
        {
            ...
            parms.mustBeOfKind = DefOfs.Gha_Syndicate_Royal_Bestower;
